...
bool QSyntaxStyle::load(QString fl) {
  ...
  while (!reader.atEnd() && !reader.hasError()) {
    ...
    if (token == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("style-scheme")) {
        ...
        if (attributes.hasAttribute("bold") &&
            attributes.value("bold") == QLatin1String("true")) {
          format.setFontWeight(QFont::Weight::Bold);
        }

        if (attributes.hasAttribute("italic") &&
            attributes.value("italic") == QLatin1String("true")) {
        ...
        if (attributes.hasAttribute("underlineStyle")) {
          ...
          if (underline == QLatin1String("SingleUnderline")) {
            s = QTextCharFormat::UnderlineStyle::SingleUnderline;
          } else if (underline == QLatin1String("DashUnderline")) {
            s = QTextCharFormat::UnderlineStyle::DashUnderline;
          } else if (underline == QLatin1String("DotLine")) {
            s = QTextCharFormat::UnderlineStyle::DotLine;
          } else if (underline == QLatin1String("DashDotLine")) {
            s = QTextCharFormat::DashDotLine;
          } else if (underline == QLatin1String("DashDotDotLine")) {
            s = QTextCharFormat::DashDotDotLine;
          } else if (underline == QLatin1String("WaveUnderline")) {
            s = QTextCharFormat::WaveUnderline;
          } else if (underline == QLatin1String("SpellCheckUnderline")) {
